...
def build_eic_dat_for_exp(exp_path: Path, NN: str, Tp: str, year: str):
    ...
    df_linetobusmap = pd.read_csv(os.path.join(data_allocation_dir, f'line_to_bus_{NN}_MW_{Tp}.csv'), header=0)
    df_line_params = pd.read_csv(os.path.join(data_allocation_dir, f'line_params_{NN}_MW_{Tp}.csv'), header=0)
...
def _parse_exp_folder_name(name: str):
    ...
    m = re.match(r'^Exp(?P<NN>\d+)(?P<UC>_[^_]*)?_MW_(?P<Tp>-?\d+)_?(?P<year>\d+)?$', name)
    if not m:
        return None
