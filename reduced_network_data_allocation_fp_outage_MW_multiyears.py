...
# =============================================================================
        ...
        # Since this MW-version later defines line limits as:
        #   MW = (1 / x) * 100 + ΔMW
        # the effective base capacity is implied by x.
        #
        # If rateA exists and is positive, we use rateA as the first
        # capacity criterion. Otherwise, we fall back to 100 / x.
        # ==========================================================
        
        df = df_branch.copy()
        
        # Create undirected line key
        df['bus_min'] = df[['fbus', 'tbus']].min(axis=1)
        df['bus_max'] = df[['fbus', 'tbus']].max(axis=1)
        df['undirected_key'] = list(zip(df['bus_min'], df['bus_max']))
        
        # Capacity used to decide which duplicate to retain
        if 'rateA' in df.columns:
            df['capacity_for_filter'] = pd.to_numeric(df['rateA'], errors='coerce')
        else:
            df['capacity_for_filter'] = np.nan
        
        # Fallback capacity based on reactance
        ...
        # Keep the largest-capacity line for each undirected pair
        df = df.sort_values(
        ...
        df = df.drop_duplicates(
            subset='undirected_key',
            keep='first'
        ).reset_index(drop=True)
        
        n_removed = len(df_branch) - len(df)
        print(
            f"NN={NN}, ΔMW={ΔMW}: removed {n_removed} duplicated undirected transmission lines "
        ...
        df_line_to_bus_out = df_line_to_bus.reset_index()
        fn_line_to_bus = f'line_to_bus_{NN}_MW_{ΔMW}.csv'
        ...
        if min_limit <= 0:
            ...
            raise ValueError(
                f"Non-positive retained line limits detected for ΔMW={ΔMW}. "
                f"Minimum limit = {min_limit:.3f} MW. "
                f"Example offending lines: {bad_lines}. "
                f"Choose a less negative trans_MW."
            )
        
        fn_line_params = f'line_params_{NN}_MW_{ΔMW}.csv'
        ...
        for UC in UC_TREATMENTS:
            for y in years:
                ...
                path=str(Path.cwd()) + str(Path('/Exp' + str(NN) + UC + '_MW_' + str(ΔMW) + '_' + str(y)))
